...
namespace Microsoft.Agents.AI.Workflows.Supervisor.Specialized;

/// <summary>
/// Base type for Supervisor Orchestration Events
/// </summary>
/// <param name="data"></param>
[JsonDerivedType(typeof(SupervisorPlanCreatedEvent))]
[JsonDerivedType(typeof(SupervisorReplannedEvent))]
[JsonDerivedType(typeof(SupervisorProgressLedgerUpdatedEvent))]
public abstract class SupervisorOrchestratorEvent(object? data) : WorkflowEvent(data)
{
}
...
public sealed class SupervisorPlanCreatedEvent(ChatMessage fullTaskLeger) : SupervisorOrchestratorEvent(fullTaskLeger)
{
    /// <summary>
...
/// </summary>
/// <param name="fullTaskLeger"></param>
public sealed class SupervisorReplannedEvent(ChatMessage fullTaskLeger) : SupervisorOrchestratorEvent(fullTaskLeger)
{
    /// <summary>
...
/// <summary>
/// Represents an update to the <see cref="SupervisorProgressLedger"/> when running a coordination round.
/// </summary>
/// <param name="progressLedger"></param>
public sealed class SupervisorProgressLedgerUpdatedEvent(SupervisorProgressLedger progressLedger) : SupervisorOrchestratorEvent(progressLedger)
{
    /// <summary>
    /// The new state of the <see cref="SupervisorProgressLedger"/>
    /// </summary>
    public SupervisorProgressLedger ProgressLedger { get; } = progressLedger;
}

/// <summary>
/// Supervisor orchestrator that defines the workflow structure.
///
/// This orchestrator manages the overall Supervisor workflow in the following structure:
///
...
internal class SupervisorOrchestrator(AIAgent managerAgent, List<AIAgent> team, TaskLimits limits, bool requirePlanSignoff)
    : ChatProtocolExecutor(nameof(SupervisorOrchestrator), s_options, declareCrossRunShareable: false)
{
    private readonly SupervisorManager _manager = new(managerAgent);
    ...
    private SupervisorTaskContext? _taskContext;
    ...
    protected override ProtocolBuilder ConfigureProtocol(ProtocolBuilder protocolBuilder)
    {
        ...
        void ConfigureRoutes(RouteBuilder routeBuilder) => routeBuilder.AddPortHandler<SupervisorPlanReviewRequest, SupervisorPlanReviewResponse>(
                "RequestPlanReview",
                this.ProcessPlanReviewAsync,
                out this._planReviewPort);
    }

    private ValueTask SubmitPlanReviewRequestAsync(SupervisorTaskContext taskContext, IWorkflowContext workflowContext, bool replanAfterStall = false)
    {
        SupervisorProgressLedger? progressLedger = taskContext.ProgressLedger;
        if (progressLedger?.IsStarted is not true)
        {
            progressLedger = null;
        }

        SupervisorPlanReviewRequest request = new(taskContext.TaskLedger!.CurrentPlan, progressLedger, replanAfterStall);

        return this._planReviewPort!.PostRequestAsync(request);
    }

    private async ValueTask ProcessPlanReviewAsync(SupervisorPlanReviewResponse response, IWorkflowContext context, CancellationToken cancellationToken)
    {
        /*
        ...
        {
            throw new InvalidOperationException("Supervisor Orchestration was not initialized correctly.");
        }

        if (this._taskContext.IsTerminated)
        {
            throw new InvalidOperationException("This Supervisor orchestration has already terminated. To process new messages, create a new workflow instance.");
        }
    ...
    private async ValueTask UpdatePlanAndDelegateAsync(SupervisorTaskContext taskContext, IWorkflowContext context, CancellationToken cancellationToken, bool replanAfterStall = false)
    {
        ...
        await context.AddEventAsync(isReplan
                                    ? new SupervisorReplannedEvent(this._fullTaskLedgerMessage)
                                    : new SupervisorPlanCreatedEvent(this._fullTaskLedgerMessage), cancellationToken).ConfigureAwait(false);
    ...
    private ValueTask DelegateToTeamAsync(SupervisorTaskContext taskContext, IWorkflowContext context, CancellationToken cancellationToken)
    {
        return this.RunCoordinationRoundAsync(taskContext, context, cancellationToken);
    }

    private async ValueTask RunCoordinationRoundAsync(SupervisorTaskContext taskContext, IWorkflowContext context, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await context.AddEventAsync(new SupervisorProgressLedgerUpdatedEvent(taskContext.ProgressLedger), cancellationToken)
                         .ConfigureAwait(false);
        }
    ...
    private async ValueTask ResetAndReplanAsync(SupervisorTaskContext taskContext, IWorkflowContext context, CancellationToken cancellationToken)
    {
    ...
    private async ValueTask PrepareFinalAnswerAsync(SupervisorTaskContext taskContext, IWorkflowContext context, CancellationToken cancellationToken)
    {
    ...
    protected internal override async ValueTask OnCheckpointingAsync(IWorkflowContext context, CancellationToken cancellationToken = default)
    {
        Task contextStateTask = this._taskContext == null
                              ? Task.CompletedTask
                              : context.QueueStateUpdateAsync(SupervisorConstants.MagenticTaskContextKey,
                                                              this._taskContext.ExportState(),
                                                              cancellationToken: cancellationToken)
                                       .AsTask();

        Task currentSpeakerTask = context.QueueStateUpdateAsync(SupervisorConstants.CurrentSpeakerStateKey,
    ...
    protected internal override async ValueTask OnCheckpointRestoredAsync(IWorkflowContext context, CancellationToken cancellationToken = default)
    {
        ...
        {
            SupervisorTaskState? state = await context.ReadStateAsync<SupervisorTaskState>(SupervisorConstants.MagenticTaskContextKey, cancellationToken: cancellationToken)
                                                    .ConfigureAwait(false);

            if (state != null)
            {
                this._taskContext = new SupervisorTaskContext(state, team, limits, []);
            }
        }

        async Task LoadCurrentSpeakerAsync()
        {
            this._currentSpeakerExecutorId = await context.ReadStateAsync<string?>(SupervisorConstants.CurrentSpeakerStateKey, cancellationToken: cancellationToken)
                                                        .ConfigureAwait(false);
        }
    }
}
